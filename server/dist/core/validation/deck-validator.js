 ...
 */
import { ValidationErrorCode } from '../types/index.js';
 ...
 */
export function validateEquipment(unit, item, cardIndex) {
    // Regra 1: Verificar se a unidade tem o slot correto
    if (!unitHasSlot(unit, item.slot)) {
        return {
            isValid: false,
            error: {
    ...
    if (itemHasAllowedTagRequirement(item)) {
        const hasRequiredTag = item.requirements.allowedTags.some((tag) => unitHasTag(unit, tag));
        if (!hasRequiredTag) {
            return {
                isValid: false,
                error: {
 ...
 */
export function validateCard(card, cardIndex, inventory) {
    const errors = [];
 ...
 */
export function validateDeck(deck, inventory) {
    const errors = [];
 ...
 */
export function calculateCardCost(unit, items) {
 ...
 */
export function calculateCardCostByIds(unitId, itemIds) {
...
//# sourceMappingURL=deck-validator.js.map